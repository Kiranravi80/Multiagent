...
def _doc_to_model(doc: dict[str, Any]) -> NetworkingModel:
    doc["id"] = str(doc.pop("_id"))
    return NetworkingModel.model_validate(doc)


class MongoNetworkingRepository(NetworkingRepository):
    """MongoDB implementation of NetworkingRepository."""

    def __init__(self, collection: AsyncIOMotorCollection) -> None:
        self._collection = collection

    async def get_by_id(self, entity_id: str) -> NetworkingModel | None:
    ...
    async def create(self, entity: NetworkingModel) -> str:
        ...
        try:
            ...
            raise DatabaseError(f"Failed to create networking entry: {exc}")

    async def update(self, entity_id: str, data: dict[str, Any]) -> NetworkingModel | None:
        try:
            ...
            raise DatabaseError(f"Failed to update networking entry: {exc}")
