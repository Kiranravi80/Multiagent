...
def _doc_to_model(doc: dict[str, Any]) -> ContentModel:
    doc["id"] = str(doc.pop("_id"))
    return ContentModel.model_validate(doc)


class MongoContentRepository(ContentRepository):
    """MongoDB implementation of ContentRepository."""

    def __init__(self, collection: AsyncIOMotorCollection) -> None:
        self._collection = collection

    async def get_by_id(self, entity_id: str) -> ContentModel | None:
    ...
    async def create(self, entity: ContentModel) -> str:
        doc = entity.to_dict()
        if "_id" in doc:
            doc.pop("_id")
        try:
            ...
            raise DatabaseError(f"Failed to create content entry: {exc}")

    async def update(self, entity_id: str, data: dict[str, Any]) -> ContentModel | None:
        try:
