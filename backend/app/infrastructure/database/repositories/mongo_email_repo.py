...
def _doc_to_model(doc: dict[str, Any]) -> EmailModel:
    doc["id"] = str(doc.pop("_id"))
    return EmailModel.model_validate(doc)


class MongoEmailRepository(EmailRepository):
    """MongoDB implementation of EmailRepository."""

    def __init__(self, collection: AsyncIOMotorCollection) -> None:
        self._collection = collection

    async def get_by_id(self, entity_id: str) -> EmailModel | None:
    ...
    async def get_by_thread_id(self, thread_id: str) -> list[EmailModel]:
        cursor = self._collection.find({"thread_id": thread_id}).sort("created_at", 1)
    ...
    async def create(self, entity: EmailModel) -> str:
        doc = entity.to_dict()
        if "_id" in doc:
            doc.pop("_id")
        try:
            ...
            raise DatabaseError(f"Failed to create email entry: {exc}")

    async def update(self, entity_id: str, data: dict[str, Any]) -> EmailModel | None:
        try:
            ...
            raise DatabaseError(f"Failed to update email: {exc}")
