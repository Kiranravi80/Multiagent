...
def _doc_to_model(doc: dict[str, Any]) -> DigestModel:
    doc["id"] = str(doc.pop("_id"))
    return DigestModel.model_validate(doc)


class MongoDigestRepository(DigestRepository):
    """MongoDB implementation of DigestRepository."""

    def __init__(self, collection: AsyncIOMotorCollection) -> None:
        self._collection = collection

    async def get_by_id(self, entity_id: str) -> DigestModel | None:
    ...
    async def get_latest_by_type(self, digest_type: str) -> DigestModel | None:
        doc = await self._collection.find_one({"type": digest_type}, sort=[("created_at", -1)])
        return _doc_to_model(doc) if doc else None
    ...
    async def create(self, entity: DigestModel) -> str:
        doc = entity.to_dict()
        if "_id" in doc:
            doc.pop("_id")
        try:
            ...
            raise DatabaseError(f"Failed to create digest entry: {exc}")

    async def update(self, entity_id: str, data: dict[str, Any]) -> DigestModel | None:
        try:
            ...
            raise DatabaseError(f"Failed to update digest: {exc}")
